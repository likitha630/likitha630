# Hi there 👋 I'm Likitha Athukuri

<p align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=6A0DAD&height=220&section=header&text=Likitha%20Athukuri&fontSize=42&fontColor=ffffff"/>
<img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=28&duration=3000&pause=1000&color=7B2CBF&center=true&vCenter=true&width=1000&lines=Software+Engineering+Student;Machine+Learning+Enthusiast;Python+Developer;Building+Scalable+Digital+Products;Open+Source+Contributor"/>
<br/>

![B.Tech](https://img.shields.io/badge/B.Tech-Information%20Technology-6A0DAD?style=for-the-badge)
![University](https://img.shields.io/badge/Vignan's%20Foundation%20for%20Science%20Technology%20and%20Research-Student-4361EE?style=for-the-badge)
![Graduation](https://img.shields.io/badge/Graduation-2027-7B2CBF?style=for-the-badge)
![Location](https://img.shields.io/badge/Location-Guntur%2C%20India-5A189A?style=for-the-badge)

<br/>

</p>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=26&pause=1000&color=8A2BE2&center=true&vCenter=true&width=700&lines=Software+Engineer;Full+Stack+Developer;Python+Developer;Always+Learning"/>
</p>

<p align="center">
<a href="https://www.linkedin.com/in/likitha-athukuri-8592a5331/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/></a>
<a href="mailto:likithaumadevi123@gmail.com"><img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail"/></a>
<a href="https://github.com/likitha630"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/></a>
</p>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=likitha630&style=for-the-badge&color=blueviolet"/>
<img src="https://img.shields.io/github/followers/likitha630?style=for-the-badge"/>
</p>

---

# About Me

- 💻 Passionate about Software Development and Technology.
- 🚀 Quick learner who enjoys exploring new technologies.
- 🌱 Currently improving my skills in Java, Python, SQL, and Web Development.
- 🎯 Looking for Software Engineer opportunities. -
- 🤝 Love building real-world projects and solving problems.

### Core Focus Areas

- Software Engineering
-  Machine Learning
- Full Stack Web Development
- Open Source Contributions
- Scalable System Design
### Open To
- Software Engineer
- Python Developer
- Product Development Projects
---

# Tech Stack

### Languages
[![My Skills](https://skillicons.dev/icons?i=python,js,html,css)](https://skillicons.dev)

### Backend & Databases
[![My Skills](https://skillicons.dev/icons?i=nodejs,flask,mongodb,mysql)](https://skillicons.dev)

### Tools
[![My Skills](https://skillicons.dev/icons?i=git,github,docker,vscode)](https://skillicons.dev)

---

# AI / ML Expertise

| Domain | Proficiency | Details |
|---|---|---|
| Machine Learning | Intermediate | Analytics Vidhya ML Certification |
| Python | Advanced | Backend development and automation |
| Full Stack | Advanced | Flask, Node.js, MongoDB |

---

# Featured Projects

<details>
<summary><b>🚀 VahanBazar — Intelligent Two-Wheeler Marketplace Platform</b></summary>

### Overview

A smart marketplace platform enabling users to discover, compare, finance, and purchase two-wheelers with integrated calculators and analytics.

| Metric | Details |
|----------|----------|
| Stack | Python Flask, MongoDB, HTML, CSS, JavaScript |
| Scale | Multi-Module Marketplace |
| Performance | Optimized Search & Filtering |
| Security | Input Validation & Authentication |
| Impact | Improved Vehicle Discovery Experience |
| Repository | Private / Hackathon Project |

#### Project Highlights

- EMI Calculator
- Fuel Cost Calculator
- Used Vehicle Listings
- Test Ride Booking
- Vehicle Comparison Engine
- Marketplace Analytics

</details>`

<details>
<summary><b>✈️ Trip Fusion — Intelligent Travel Management Platform</b></summary>

### Overview

A comprehensive travel management platform designed to simplify trip planning by offering destination discovery, hotel reservations, transportation booking, secure payments, and personalized travel experiences through an intuitive web interface.

| Metric | Details |
|----------|----------|
| Stack | Node.js, MongoDB, HTML, CSS, JavaScript |
| Scale | End-to-End Travel Management Platform |
| Performance | Optimized Booking Workflow & Database Queries |
| Security | Role-Based Authentication & Secure Payment Integration |
| Impact | Simplified Travel Planning and Booking Experience |
| Repository | Academic Full Stack Project |

#### Project Highlights

- Destination Discovery
- Hotel & Resort Booking
- Transportation Reservation
- Secure User Authentication
- Online Payment Integration
- Booking Confirmation System
- Personalized Travel Recommendations
- Responsive User Dashboard
- Admin Management Portal
- Booking History & Trip Tracking

</details>

# Experience
## HackXlerate 2025 — National Hackathon Finalist

**Date Range:** 2025

Selected among the Top 7 teams nationwide and qualified for the Grand Finale with VahanBazar, an intelligent two-wheeler marketplace platform.

### Scope of Work

- Designed database schemas
- Managed data modeling
- Supported backend integration
- Assisted analytics workflows
- Contributed to technical presentations

**Skills**

`MongoDB` `Database Design` `Flask` `Problem Solving` `Team Collaboration`

## Coordinator — PR & DM, Student Activity Council (SAC)

**Date Range:** 2025 – Present

Serving as a Coordinator in the **Public Relations & Digital Media (PR & DM)** team of the **Student Activity Council (SAC)**, contributing to the planning, promotion, and successful execution of university events while strengthening communication and digital outreach.

### Scope of Work

- Coordinated with multiple student teams for event planning and execution.
- Managed digital promotions and public relations activities across various platforms.
- Assisted in organizing university-level technical, cultural, and academic events.
- Collaborated with faculty members, student volunteers, and stakeholders.
- Supported event scheduling, coordination, and smooth operational workflows.
- Helped increase student engagement through effective communication and promotional campaigns.

**Skills**

`Leadership` `Public Relations` `Digital Media` `Event Coordination` `Communication` `Team Collaboration` `Content Promotion` `Planning`

---

# Achievements

| Recognition | Details |
|---|---|
| 🏆 Department Academic Topper | Maintained top academic performance in the Information Technology department throughout the B.Tech program. |
| 🚀 ByteXL Hackathon Grand Finalist | Reached the Grand Finale through innovative problem-solving, project development, and teamwork. |
| 💻 Full Stack Projects | Built production-style web applications using Flask, Node.js and MongoDB. |

---

# Certifications

- Cambridge English: Preliminary (PET) – Grade C (B1 Level)
- NPTEL – Privacy and Security in Online Social Media (IIT)
- Analytics Vidhya – Machine Learning Certification for Beginners

---

# Coding Profiles

<div align="center">

<a href="https://leetcode.com/u/Athukuri_Likitha15">
<img src="https://img.shields.io/badge/LeetCode-Problem%20Solver-FFA116?style=for-the-badge&logo=leetcode&logoColor=white"/>
</a>

<a href="https://www.geeksforgeeks.org/profile/likithaumvepu">
<img src="https://img.shields.io/badge/GeeksforGeeks-Coding%20Practice-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white"/>
</a>

<a href="https://www.codechef.com/users/likithaumadevi">
<img src="https://img.shields.io/badge/CodeChef-Competitive%20Coding-5B4638?style=for-the-badge&logo=codechef&logoColor=white"/>
</a>

</div>

---

# GitHub Analytics

<p align="center">
<img height="170" src="https://github-readme-stats.vercel.app/api?username=likitha630&show_icons=true&theme=tokyonight"/>
<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=likitha630&layout=compact&theme=tokyonight"/>
</p>

<p align="center">
<img src="https://streak-stats.demolab.com?user=likitha630&theme=tokyonight"/>
</p>

---

# Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=likitha630&theme=tokyo-night&hide_border=true"/>

</div>
---

---


---
# Connect

<div align="center">

<a href="mailto:likithaumadevi123@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/likitha-athukuri-8592a5331/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/likitha630/">
<img src="https://img.shields.io/badge/GitHub-Follow-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</div>

---

<div align="center">

*"Learning never exhausts the mind; it empowers innovation."*

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=150&color=0:4B0082,30:6A0DAD,60:7B2CBF,100:4361EE"/>

</div>
